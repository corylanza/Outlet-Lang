...
namespace Outlet.Checking {
	public class Checker : IVisitor<Type> {
        ...
        public static Type Get(int level, string s) => Scopes.Peek().GetType(level, s);
		...
		public static Type Cast(Type from, Type to, string message = "cannot convert type {0} to type {1}") {
		...
		public Type Visit(ClassDeclaration c) {
			if(!DoImpl.Peek()) {
				...
				if(c.SuperClass != null) {
					if(c.SuperClass.Accept(this) is MetaType) Error("cannot extend anything other than a class");
					else {
                        Operand p = Get(c.SuperClass.resolveLevel, c.SuperClass.Name);
                        if(p is MetaType meta && meta.HiddenType is Class pt) parent = pt;
						else Error("cannot extend anything other than a class");
					}
				}
				Define(new MetaType(new ProtoClass(c.Name, parent, instances, statics)), c.Name);
				EnterScope();
				...
				if(c.SuperClass != null) {
                    Operand p = Get(c.SuperClass.resolveLevel, c.SuperClass.Name);
                    if (p is MetaType meta && meta.HiddenType is ProtoClass pt) parent = pt;
		...
		public Type Visit(FunctionDeclaration f) {
			if(!DoImpl.Peek()) {
				...
				(Type type, string id)[] args = f.Args.Select(arg => {
					...
					// if(curarg is MetaType) DefineType(new ProtoClass(arg.ID, null, null, null), arg.ID);
				...
				if(f is ConstructorDeclaration && body != null) return Error("constructor cannot return value");
		...
		public Type Visit(VariableDeclaration v) {
			...
			if(init is MetaType meta) Define(meta, v.Decl.ID);
		...
		public Type Visit(Access a) {
            ...
            if (elem is MetaType meta) {
                if (meta.HiddenType is Class c && a.Index.Length > 0)
                {
                    return Error("Generics not supported yet");
                }
                return new MetaType(new ArrayType(meta.HiddenType));
            }
            if (a.Index.Length != 1)
		...
		public Type Visit(As a) {
			...
			if(r is MetaType castedTo) return castedTo.HiddenType;
		...
		public Type Visit(Call c) {
			...
			if(calltype is MetaType constructor) {
				if(constructor.HiddenType is ICheckableClass cl) calltype = cl.GetStaticType("");
				else return Error("type " + constructor.HiddenType + " is not instantiable");
			}
        ...
        {
            Type decl = d.Type.Accept(this);
            if (decl is MetaType meta) return meta.HiddenType;
		...
		public Type Visit(Deref d) {
			...
			if(inst is MetaType staticClass) {
				if(staticClass.HiddenType is ICheckableClass c) return c.GetStaticType(d.Right);
				return Error("type " + staticClass.HiddenType.ToString() + " does not contain static field: " + d.Right);
			}
		...
		public Type Visit(Is i) {
            ...
            if (r is MetaType) return Primitive.Bool;
            return Error("the right side of an is expression must be a type, found: " + r.ToString());
		}
		...
		public Type Visit(Lambda l) {
			...
			if(args is MetaType a && res is MetaType r) {
        ...
        public Type Visit(UsingStatement u)
        {
            ...
            if (used is MetaType meta && meta.HiddenType is ICheckableClass c)
